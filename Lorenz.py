...
def plot_multi_results(t, X1, Y1, X2, Y2):
	fig, ax = plt.subplots( 3 , 2 , sharex = True, figsize=(10,5) )

	ax[0][0].plot(t  , X1[:,0], label='Full simulation 1' )
	ax[0][0].plot(t , Y1[:,0], linestyle='--', label='Identified model 1')
	ax[0][0].set_ylabel('x(t)')
	ax[0][0].legend(loc='upper center', bbox_to_anchor=(.5, 1.33), ncol=2, frameon=False )

	ax[1][0].plot(t, X1[:,1])
	ax[1][0].plot(t ,Y1[:,1], ls='--')
	ax[1][0].set_ylabel('y(t)')

	ax[2][0].plot(t, X1[:,2])
	ax[2][0].plot(t ,Y1[:,2], ls='--')
	ax[2][0].set_ylabel('z(t)')
	ax[2][0].set_xlabel('Time')
	ax[2][0].set_xlim(0, 20)

	ax[0][1].plot(t  , X2[:,0], label='Full simulation 2' )
	ax[0][1].plot(t , Y2[:,0], linestyle='--', label='Identified model 2')
	ax[0][1].set_ylabel('x(t)')
	ax[0][1].legend(loc='upper center', bbox_to_anchor=(.5, 1.33), ncol=2, frameon=False )

	ax[1][1].plot(t, X2[:,1])
	ax[1][1].plot(t ,Y2[:,1], ls='--')
	ax[1][1].set_ylabel('y(t)')

	ax[2][1].plot(t, X2[:,2])
	ax[2][1].plot(t, Y2[:,2], ls='--')
	ax[2][1].set_ylabel('z(t)')
	...
	ax.plot(X1[-2500:,0], X1[-2500:,1], X1[-2500:,2])
	ax.plot(Y1[-2500:,0], Y1[-2500:,1], Y1[-2500:,2], ls='--')
	ax.axis('equal')

	ax = fig.add_subplot(1, 2, 2, projection='3d')
	ax.plot(X2[-2500:,0], X2[-2500:,1], X2[-2500:,2])
	ax.plot(Y2[-2500:,0], Y2[-2500:,1], Y2[-2500:,2], ls='--')
	ax.axis('equal')
